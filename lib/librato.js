...
var flush_stats = function librato_flush(ts, metrics)
{
  ...
  for (key in metrics.timers) {
    ...
    // Build log values
    var old_timingUnit = timingUnit;
    if (old_timingUnit) {
      timingUnit = "log("+old_timingUnit+")";
    }
    var log_gauge = timer_gauge_pct(key, logVals, 100, ".log");
    timingUnit = old_timingUnit;
    if (log_gauge) {    
      // Logs are not counted in numStats
      addMeasure('gauge', log_gauge, false);
    }
...
exports.init = function librato_init(startup_time, config, events)
{
  ...
  if (config.librato) {
    ...
    if (timerUnit) {
      timerUnit = config.librato.timerUnit;
    }
    if (timerFunction) {
      timerFunction = config.librato.timerFunction;
    }
