...
// Unit to use for timers, default to ms
//
var timerUnit = "Milliseconds";

// Function to use for aggregating measurements for same time interval.
// Must be 'average' or sum (defaults to 'sum').
//
var timerFunction = "sum";
...
var timer_gauge_log = function(gauge) {
  return {
    name: sanitize_name(gauge.name+".log"),
    count: gauge.count,
    sum: Math.log(gauge.sum),
    sum_squares: Math.log(gauge.sum_squares),
    min: Math.log(gauge.min),
    max: Math.log(gauge.max),
    attributes: { 
      display_units_long: "log("+gauge['attributes'].display_units_long+")",
      summarize_function: gauge['attributes'].summarize_function
    }
  };
};

var timer_gauge_pct = function(timer_name, values, pct, suffix, timerUnit, timerFunction)
{
  ...
  return {
    ...
    max: max,
    attributes: { 
      display_units_long: timerUnit,
      summarize_function: timerFunction
    }
  };
}
...
var flush_stats = function librato_flush(ts, metrics)
{
  ...
  for (key in metrics.timers) {
    ...
    var gauge = timer_gauge_pct(key, sortedVals, 100, null, timerUnit, timerFunction);
    var logGauge = timer_gauge_log(gauge);
    ...
    if (logGauge) {
      addMeasure('gauge', logGauge, false);
    }
    ...
    for (pKey in metrics.pctThreshold) {
      ...
      gauge = timer_gauge_pct(key, sortedVals, pct, "." + pct, timerUnit, timerFunction);
      if (gauge) {
